...
if (
    ...
    (int) $validatedBotCode->bot_id !== (int) $bot->id
) {
    ...
    return redirect()
        ...
        ->with(
            'error',
            'Please load a valid Bot Code before launching a bot.'
        );
}
...
$prefilledAmount =
    session('validated_bot_amount');
        ...
        $runningSubscription = $user
            ->botSubscriptions()
            ->where('simulation_status', 'running')
         ...
         *
         * Running simulation gets priority.
         */
    ...
    public function subscribe(Request $request)
    {
        ...
        try {
             ...
             * ====================================================
             *
             * The user never receives this target directly.
             */
            $minFinalProfit =
                (float) $bot->min_final_profit;

            $maxFinalProfit =
                (float) $bot->max_final_profit;
            ...
            $minUnits = (int) round(
                $minFinalProfit * 100000000
            );

            $maxUnits = (int) round(
                $maxFinalProfit * 100000000
            );

            $simulationTarget = round(
                random_int(
                    $minUnits,
                    $maxUnits
                ) / 100000000,
            ...
            try {
                ...
                Log::warning(
                    'Could not attach simulation trade.',
                    [
                        'subscription_id' =>
                            $subscription->id,
